                ...
                <div class="p-6 text-gray-900">
                    ...
                    <div class="bg-gray-50 p-4 rounded-lg mb-6">
                        <form method="GET" action="{{ route('manager.users') }}" class="flex flex-col md:flex-row gap-4">
                            ...
                            <div class="flex gap-2">
                                ...
                                <a href="{{ route('manager.users') }}" class="px-4 py-2 bg-gray-300 text-gray-700 rounded-md hover:bg-gray-400">
                                    Clear
                                </a>
                            </div>
                        </form>
                    ...
                    <div class="flex justify-between items-center mb-4">
                        <div class="text-sm text-gray-600">
                            @if(request('search') || request('role'))
                                ...
                                @endif
                            @else
                                Showing {{ $users->count() }} of {{ $users->total() }} total users
                            @endif
                        </div>
                    ...
                    <div class="mt-8 grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div class="bg-blue-50 p-4 rounded-lg">
                            ...
                            <p class="text-blue-500 text-xs mt-1">Count: {{ $allUsers->where('role', 'manager')->count() }}</p>
                        </div>
                        <div class="bg-orange-50 p-4 rounded-lg">
                            <h4 class="font-semibold text-orange-800">Gudang</h4>
                            <p class="text-orange-600 text-sm">Inventory and stock management</p>
                            <p class="text-orange-500 text-xs mt-1">Count: {{ $allUsers->where('role', 'gudang')->count() }}</p>
                        </div>
                        <div class="bg-gray-50 p-4 rounded-lg">
                            <h4 class="font-semibold text-gray-800">Member</h4>
                            <p class="text-gray-600 text-sm">Limited access</p>
                            <p class="text-gray-500 text-xs mt-1">Count: {{ $allUsers->where('role', 'member')->count() }}</p>
                        </div>
                    </div>
